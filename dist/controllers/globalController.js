...
exports.uploadImages = (pictures, userID, route) => {
  return new Promise(function (resolve, reject) {
    ...
    pictures.forEach((picture, i) => {
      let uploadPath = path.normalize(process.cwd() + `/public/${route}/`); // In case the '/uploads' directory doesn't exist
      ...
      var base64Image = picture.img.replace(/^data:image\/[a-z]+;base64,/, ""); // let data = Buffer.from(base64Image, 'base64');
      // Creating the name for the file --> random + userID + timestamp + extension

      let filename = Math.floor(Math.random() * 9999999 + 1111111).toString() + '-' + userID + '-' + new Date().getTime().toString() + '.' + picture.filetype; // Writing the image to filesystem
