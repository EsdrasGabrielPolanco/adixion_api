...
exports.getShoppingCar = async (req, res) => {
  ...
  try {
    let supplishoppingCar = await ShoppingCar.findOne({
      _id: id
    });
    res.status(200).json({
      shoppingCar
    ...
    try {
      payment.products.forEach(async element => {
        ...
        if (quantity > 0) {
          ...
          payment = await payment.save().then(result => {
            res.status(201).json({
...
exports.listAll = async (req, res) => {
  try {
    let payments = await Payment.find().populate(['user', 'products.product']).exec();
    res.status(200).json({
      payments
    });
  } catch (error) {
